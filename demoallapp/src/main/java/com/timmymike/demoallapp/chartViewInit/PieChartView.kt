...
import com.timmymike.demoallapp.UserRecordModel
import com.timmymike.demoallapp.dialog_base.DateTool
import com.timmymike.demoallapp.dialog_base.toDate
import com.timmymike.demoallapp.dialog_base.toDateNonNull
import com.timmymike.demoallapp.dialog_base.toString
import com.timmymike.radarcharttrial.dialog_base.logi
import java.text.DecimalFormat
import java.util.*
import kotlin.collections.ArrayList

class PieChartView (private val context: Context){
...
//    }
    ...
    fun initPieChartView(chart: PieChart, dataList: List<UserRecordModel.Data>) {
        ...
        for (data in dataList) {
            if (cacuLationArray.none { it.departmentName == data.department }) { //舊的沒有，要新增
                cacuLationArray.add(CacuLationData().apply {
                    ...
                    stayTime = (data.finishedWorkAt?.toDateNonNull(timeFormate)?.time?:0L) - (data.startedWorkAt?.toDateNonNull(timeFormate)?.time?:0L)
                })
            } else {//舊的已有，要加上去
...
//        Log.i(TAG,"")
    ...
    /**==========================產生測試資料程式碼如下==========================*/
     ...
     fun getPieChartExampleData(dataSize: Int): TreeSet<UserRecordModel.Data> {
