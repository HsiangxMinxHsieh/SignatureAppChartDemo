...
public class JavaVisibleLineRender extends LineRadarRenderer {
    ...
    public JavaVisibleLineRender(LineDataProvider chart, ChartAnimator animator,
    ...
    protected void drawLinear(Canvas c, ILineDataSet dataSet) {
        ...
        if (dataSet.getColors().size() > 1) {
            ...
            VisibleEntry e1, e2;
            e1 = (VisibleEntry)dataSet.getEntryForIndex(mXBounds.min);
            if (e1 != null) {
                int j = 0;
                for (int x = mXBounds.min; x <= mXBounds.range + mXBounds.min; x++) {
                    e1 = (VisibleEntry)dataSet.getEntryForIndex(x == 0 ? 0 : (x - 1));
                    e2 = (VisibleEntry)dataSet.getEntryForIndex(x);
    ...
    private void generateFilledPath(final ILineDataSet dataSet, final int startIndex, final int endIndex, final Path outputPath) {
        ...
        filled.reset();
        final VisibleEntry entry = (VisibleEntry)dataSet.getEntryForIndex(startIndex);
        ...
        // create a new path
        VisibleEntry currentEntry = null;
        VisibleEntry previousEntry = entry;
        boolean closed = false;
        for (int x = startIndex + 1; x <= endIndex; x++) {
            currentEntry =(VisibleEntry) dataSet.getEntryForIndex(x);
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                for (int j = 0; j < positions.length; j += 2) {
                    ...
                    if (!mViewPortHandler.isInBoundsLeft(x) || !mViewPortHandler.isInBoundsY(y))
                        continue;
                    VisibleEntry entry = (VisibleEntry)dataSet.getEntryForIndex(j / 2 + mXBounds.min);
    ...
    protected void drawCircles(Canvas c) {
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            for (int j = mXBounds.min; j <= boundsRangeCount; j++) {
                VisibleEntry e = (VisibleEntry)dataSet.getEntryForIndex(j);
                if (e == null) break;
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (Highlight high : indices) {
            ...
            if (set == null || !set.isHighlightEnabled())
                continue;
            VisibleEntry e = (VisibleEntry)set.getEntryForXValue(high.getX(), high.getY());
