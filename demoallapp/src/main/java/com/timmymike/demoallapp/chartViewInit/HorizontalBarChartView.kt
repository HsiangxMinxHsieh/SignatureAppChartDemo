...
class HorizontalBarChartView (private val context: Context){
...
//    }
    ...
    /**依照dataSize取得xLabel的方法*/
     fun getXlabels(dataSize: Int,reverse:Int): Array<String> {
        val result = Array<String>(dataSize) { "" }
        for (i in 0 until dataSize) {
            result[i] = DateUtils.getMonthString(i, DateUtils.LENGTH_SHORTER)
        }
//        logi(TAG,"回傳前的xLabels是===>$result")
    ...
    /**依照dataSize取得隨機資料的方法*/
     fun getHorizontalBarExampleData(dataSize: Int): ArrayList<Int> {
...
//    }
     ...
     fun initHorizontalBarCharView(chart: HorizontalBarChart, datas: ArrayList<Int>, xLabels: Array<String>) {
